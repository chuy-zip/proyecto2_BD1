...
async function getComplaintsByProduct(startDate, endDate){
    try {
        
        let complaints = await fetch(`http://localhost:8080/reports/product-complaints?startDate=${startDate}&endDate=${endDate}`)
        if (!complaints.ok){
            throw new Error("Failed tofetch complaints")
        }
        let json_list = await complaints.json()
        ...
        json_list.forEach((value, index) => {
            const key = `${value.id_producto}-${value.nombre}`
